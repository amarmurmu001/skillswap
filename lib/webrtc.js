...
export default class CallEngine {
  ...
  async join(roomName, userId, displayName) {
    ...
    this.channel
      ...
      .on('broadcast', { event: 'chat' }, ({ payload }) => {
        if (payload.userId !== this.userId) this._emit('chat-message', payload);
      })
      .on('broadcast', { event: 'screen-change' }, ({ payload }) => {
        if (payload.userId !== this.userId) this._emit('peer-screen-change', payload);
      });
  ...
  async leave() {
    if (this._joined) await this._broadcast('leave', { userId: this.userId });
    this._cleanup();
  }
