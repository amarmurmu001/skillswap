...
/** Returns sorted perfect/partial matches (score > 0) */
export async function getMatchSuggestions(currentUser) {
  const allUsers = await getUsers();
  return rankUsers(currentUser, allUsers);
}

/** Returns all other users ranked by match score (score can be 0) */
export async function getBrowseUsers(currentUser) {
  const allUsers = await getUsers();
  return allUsers
    .filter(u => u.id !== currentUser.id)
    .map(other => {
