...
export default function ChatPage() {
  ...
  const activeMatches = getMatchesForUser(user.id).filter(m => m.status === 'active');

  const conversations = activeMatches.map(m => {
    const otherId = m.userAId === user.id ? m.userBId : m.userAId;
    const other   = getUserById(otherId);
    const messages = getMessagesByMatch(m.id);
  ...
  const activeConversation = activeMatchId
    ? conversations.find(c => c.match.id === activeMatchId)
    : conversations[0];
  ...
  const currentMatch  = activeConversation?.match;
  const currentOther  = activeConversation?.other;
      ...
      <div style={{
        ...
        <div style={{ padding: '1.25rem 1.25rem 0.875rem', borderBottom: '1px solid rgba(99,102,241,0.1)' }}>
          <h2 style={{ fontFamily: 'Outfit,sans-serif', fontWeight: 700, fontSize: '1rem' }}>
            Messages
            <span style={{ marginLeft: 8, background: 'rgba(99,102,241,0.15)', color: '#818cf8', borderRadius: 9999, padding: '0 8px', fontSize: '0.72rem' }}>
            ...
            return (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ color: '#6b7280', fontSize: '0.78rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 2 }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ position: 'relative' }}>
              <img src={currentOther.avatar} alt={currentOther.name} style={{ width: 44, height: 44, borderRadius: '50%', background: '#1a1a27' }} />
              {currentOther.isOnline && <div style={{ position: 'absolute', bottom: 1, right: 1, width: 10, height: 10, background: '#4ade80', borderRadius: '50%', border: '2px solid #111118' }} />}
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontWeight: 700, fontSize: '0.975rem' }}>{currentOther.name}</div>
              <div style={{ color: currentOther.isOnline ? '#4ade80' : '#6b7280', fontSize: '0.78rem' }}>
                {currentOther.isOnline ? '● Online' : '○ Offline'}
              </div>
            </div>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <a
                href={`https://meet.jit.si/skillswap-${currentMatch.id}`}
                target="_blank"
              ...
              <a href="/sessions" className="btn-secondary" style={{ fontSize: '0.8rem', padding: '0.5rem 0.875rem', textDecoration: 'none' }}>
                📅 Schedule
              </a>
            </div>
          </div>

          <ChatBox matchId={currentMatch.id} currentUserId={user.id} otherUser={currentOther} />
        </div>
      )}
    </div>
  );
