...
import { getMatchesForUser, getUserById, getSessionsForUser, createSession, updateSession } from '@/lib/data';
...
export default function SessionsPage() {
  ...
  async function load() {
    ...
    setSessions(s);
    setMatches(m.filter(m => m.status === 'active'));
    setLoading(false);
  }
          ...
          <form onSubmit={handleCreate} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1.25rem' }}>
            <div>
              ...
              <select className="input-field" value={form.matchId} onChange={e => setForm(p => ({ ...p, matchId: e.target.value }))} required style={{ cursor: 'pointer' }}>
                ...
                {matches.map(m => <MatchOption key={m.id} match={m} userId={user.id} />)}
              </select>
            </div>
            <div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {byStatus(tab).length === 0 ? (
            ...
            <SessionRow key={session.id} session={session} userId={user.id} matches={matches} tab={tab} onComplete={handleComplete} />
          ))}
        </div>
      )}
    </div>
...
function MatchOption({ match, userId }) {
  const [name, setName] = useState('Loading...');
  useEffect(() => {
    const otherId = match.userAId === userId ? match.userBId : match.userAId;
    getUserById(otherId).then(u => setName(u?.name || otherId));
  }, [match, userId]);
  return <option value={match.id}>{name}</option>;
}

function SessionRow({ session, userId, matches, tab, onComplete }) {
  ...
  useEffect(() => {
    ...
    if (otherId) getUserById(otherId).then(setOther);
  }, [session, matches, userId]);

  const isPast = new Date(session.scheduledAt) < new Date();

  return (
    <div className="glass-card" style={{ padding: '1.5rem', display: 'flex', gap: '1.25rem', alignItems: 'center', flexWrap: 'wrap' }}>
      <div style={{ width: 56, height: 56, flexShrink: 0, background: 'linear-gradient(135deg,rgba(99,102,241,0.2),rgba(217,70,239,0.15))', border: '1px solid rgba(99,102,241,0.25)', borderRadius: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.5rem' }}>
        {tab === 'upcoming' ? '📅' : '✅'}
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.625rem', flexShrink: 0 }}>
        {tab === 'upcoming' && (
          <>
            <a href={session.meetingLink} target="_blank" rel="noopener noreferrer" className="btn-primary" style={{ fontSize: '0.82rem', padding: '0.5rem 1rem', textDecoration: 'none' }}><span>🎥 Join Call</span></a>
