...
export default function ChatBox({ matchId, currentUserId, otherUser }) {
  ...
  useEffect(() => {
    if (!matchId) return;
    setMessages(getMessagesByMatch(matchId));
  }, [matchId]);
  ...
  async function sendMessage(e) {
    ...
    setSending(true);
    setInput('');
    const msg = addMessage(matchId, currentUserId, text);
            ...
            <div key={msg.id} style={{ display: 'flex', flexDirection: isMine ? 'row-reverse' : 'row', gap: '0.5rem', alignItems: 'flex-end' }}>
              {!isMine && (
                <div style={{ width: 28, flexShrink: 0 }}>
                  {showAvatar && (
                    <img
                      src={sender?.avatar || otherUser?.avatar}
                      alt=""
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: isMine ? 'flex-end' : 'flex-start', maxWidth: '72%' }}>
                <div
                  ...
                  style={{
                    ...
                    background: isMine ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : 'rgba(26,26,39,0.9)',
                    color: isMine ? '#fff' : '#f0f0ff',
                    border: isMine ? 'none' : '1px solid rgba(99,102,241,0.15)',
                  }}
                >
                  {msg.content}
                </div>
                <div style={{ fontSize: '0.68rem', color: '#6b7280', paddingInline: 4 }}>
